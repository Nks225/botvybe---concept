...
Сегменты:
Основной сегмент: молодые пользователи 18–27 лет

Это студенты, молодые специалисты и активные пользователи Telegram, которые:
  постоянно пользуются мобильным телефоном;
  привыкли к быстрым сценариям потребления контента;
  готовы к знакомствам в формате «свайп + короткое действие»;
  не хотят проходить длинную регистрацию в классических dating-сервисах;
  комфортно чувствуют себя внутри Telegram и доверяют ему как основной платформе общения.

Вторичный сегмент: пользователи 27–35 лет

Это работающие пользователи, которым важны:
  скорость входа в сервис;
  минимальное количество лишних действий;
  локальный подбор анкет;
  более “спокойный” и менее перегруженный формат знакомств по сравнению с крупными dating-платформами.

Поведенческий сегмент: пользователи, ищущие знакомства в своем городе
  Сервис ориентирован не на абстрактную широкую аудиторию, а на пользователей, которым важно знакомиться с людьми рядом. Это подтверждается логикой продукта: в основе ленты лежит город пользователя и        близкий возрастной диапазон. Значит, приложение в первую очередь подходит людям, заинтересованным в реальном, локальном общении, а не только в онлайн-переписке.

Поведенческий сегмент: пользователи, ценящие низкий порог входа

Это люди, которые:
  не хотят скачивать отдельное приложение;
  не готовы тратить время на длинный onboarding;
  предпочитают запускать продукт сразу из Telegram;
  быстрее вовлекаются, если можно начать пользоваться без сложной регистрации.

Итоговая целевая аудитория проекта

Ключевая целевая аудитория:

мужчины и женщины 18–35 лет;
активные пользователи Telegram;
жители городов, заинтересованные в локальных знакомствах;
пользователи, которым важны быстрый вход, мобильный формат, простая механика взаимодействия и приватность.
Приоритетное ядро аудитории

Наиболее перспективное ядро аудитории:
пользователи 18–27 лет;
Telegram-native аудитория;
люди, привыкшие к mobile-first продуктам;
пользователи, которым нужен простой и быстрый формат знакомства без отдельной регистрации.
## Структура сайта
